...
# def ordered_encode_columns(data_df, fillCols=None):
#     if fillCols is None:
...
#             data_df[col] = data_df[col].fillna("ZZZZZ")
...
class ContinuousMissingValueFiller:
    ...
    # 使用有序数字编码方式对分类特征编码（考虑数值大小意义）
    def ordered_encode_columns(self):

        data_df = self.original_data
    ...
    def fill_method_contrast(self, fillCols=None, fill_methods=None):
        """
    ...
    def basic_fill_method(self, fillCols=None, fill_methods=None):
        """常规缺失值填补方法
        ...
        assert len(fillCols) == len(fill_methods), "The column name list and corresponding method list do not have the same length"
    ...
    def kmeans_group_fill_method(self, corrCol, fillCols=None, fillMethod="ffill", n_clusters=2, only_missValue=True):
        """K-means分层均值缺失值填补方法
        ...
        for fillCol in fillCols:
            for cluster in clusters_:
                ...
                # 计算当前分组中缺失值列的均值
                method = getattr(fillCol_s, fillMethod)
                cluster_mean = method()
                ...
                # 使用分组均值填充缺失值或0值
                if fillMethod == "mode":
                    data_df.loc[bool_index, fillCol] = cluster_mean[cluster]
                else:
    ...
    def sequential_kmeans_group_fill_method(self, clusterCol, fillCols=None, fill_method="ffill", n_clusters=2):
        """K-means分层均值缺失值填补方法
       ...
       """
        ...
        for i in range(cluster_count.shape[0]):
            ...
            df_null_index = pd.Index(fillCols)
    ...
    def lagrange_fill_method(self, fillCols=None):
        """拉格朗日插补缺失值方法
    ...
    def model_fit_fill_method(self, model, fillcols=None, fill0=False):
        """
    ...
    def iterate_model_fit_fill_method(self, model, fillcols=None, fill0=False):
...
# class CategoricalMissingValueFiller:
#     def __init__(self, data_df, fillCols="", fill_methods=""):
#         self.original_data = data_df.copy()
#         self.missing_value_dF = data_df.copy()
#         self.fillCols = fillCols    # 缺失值填补列名称，列表类型
#         self.fill_methods = fill_methods    # 缺失值填补方法
#
#     # 某个特征常规缺失值填补方法对比, 最好一次对比一个特征
#     def fill_method_contrast(self, fillCols=None, fill_methods=None):
#         """
...
#         fill_method_Df = pd.DataFrame(fill_method_Df)  # 将填补后的特征列转换为DataFrame对象
#
#         legend_ = ["original_data", "filled_data"]
#         fig = plt.figure(dpi=128, figsize=(10, 20))
#         for k, fillCol in enumerate(fillCols_):
#             ax = fig.add_subplot(math.ceil(len(fillCols_) / 2), 2, k + 1)
#             # 获取类别和对应的计数数据
#             categories = original_data[fillCol[:fillCol.find('_')]].value_counts().index
#             original_counts = original_data[fillCol[:fillCol.find('_')]].value_counts().values
#             filled_counts = fill_method_Df[fillCol[:fillCol.find('_')]].value_counts().values
#             # 绘制原始数据柱状图
#             ax.bar(categories, original_counts, color='blue', alpha=0.5)
#             # 绘制填补后数据柱状图
#             ax.bar(categories, filled_counts, color='green', alpha=1, bottom=original_counts)
#             # 添加图例
#             ax.legend(legend_, loc='best')
#             # 设置轴标签和标题
#             ax.set_xlabel('Category')
#             ax.set_ylabel('Count')
#             ax.set_title(fillCol[:fillCol.find('_')] + ' Bar Chart')
#         # 显示图形
#         plt.show()
#
#         return fill_method_Df  # 返回填补后的特征列和填补后特征列的列名
...
#             categories = original_data[fillCol[:fillCol.find('_')]].value_counts().index
#             original_counts = original_data[fillCol[:fillCol.find('_')]].value_counts().values
#             filled_counts = missing_value_dF[fillCol[:fillCol.find('_')]].value_counts().values
#             # 绘制原始数据柱状图
#             ax.bar(categories, original_counts, color='blue', alpha=0.5)
#             # 绘制填补后数据柱状图
#             ax.bar(categories, filled_counts, color='green', alpha=1, bottom=original_counts)
...
#         return missing_value_dF
#
#     def kmeans_group_fill_method(self, corrCol, fillCols=None, fillMethod="mode", n_clusters=2, only_missValue=True):
#         """K-means分层均值缺失值填补方法
#
#             :param fillMethod:
#             :param fillCols:
...
#         original_data = self.original_data
#         if fillCols is None:
#             fillCols = self.fillCols    # 缺失值列（A），列表类型
#
#         # 使用属性B（与缺失值列A相关性最强）的众数均值填充缺失值列A的缺失值
#         data_df[corrCol].fillna(data_df[corrCol].mode().mean(), inplace=True)
#
...
#             # 获取类别和对应的计数数据
#             categories = original_data[fillCol].value_counts().index
#             original_counts = original_data[fillCol].value_counts().values
#             filled_counts = data_df[fillCol].value_counts().values
#             # 绘制原始数据柱状图
#             ax.bar(categories, original_counts, color='blue', alpha=0.5)
#             # 绘制填补后数据柱状图
#             ax.bar(categories, filled_counts, color='green', alpha=1, bottom=original_counts)
#             # 添加填补前数据的数值标签
#             for i, orig_count in enumerate(original_counts):
#                 ax.text(categories[i], orig_count, f"{orig_count}", ha='center', va='bottom')
#             # 添加填补后数据的数值标签
#             for i, fill_count in enumerate(filled_counts):
#                 ax.text(categories[i], original_counts[i] + fill_count, f"{fill_count}", ha='center', va='bottom')
#             # 添加图例
#             ax.legend(legend_, loc='best')
#             # 设置轴标签和标题
#             ax.set_xlabel('Category')
...
#             categories = original_data[fillCol[:fillCol.find('_')]].value_counts().index
#             original_counts = original_data[fillCol[:fillCol.find('_')]].value_counts().values
#             filled_counts = data_full_fill[fillCol[:fillCol.find('_')]].value_counts().values
#             # 绘制原始数据柱状图
#             ax.bar(categories, original_counts, color='blue', alpha=0.5)
#             # 绘制填补后数据柱状图
#             ax.bar(categories, filled_counts, color='green', alpha=1, bottom=original_counts)
...
#             categories = original_data[fillCol[:fillCol.find('_')]].value_counts().index
#             original_counts = original_data[fillCol[:fillCol.find('_')]].value_counts().values
#             filled_counts = data_df[fillCol[:fillCol.find('_')]].value_counts().values
#             # 绘制原始数据柱状图
#             ax.bar(categories, original_counts, color='blue', alpha=0.5)
#             # 绘制填补后数据柱状图
#             ax.bar(categories, filled_counts, color='green', alpha=1, bottom=original_counts)
...
#             ax.set_xlabel('Category')
#             ax.set_ylabel('Count')
#             ax.set_title(fillCol + ' Bar Chart')
#         # 显示图形
#         plt.show()
#
#         return data_df
...
#         original_data = self.original_data
#         if fillcols is None:
#             fillcols = self.fillCols    # 进行缺失值和异常值填补（如0值）的列索引名称，列表形式
#
#         """1. 使用算法拟合插补缺失值NaN"""
#         # 获取含有缺失值NaN的列索引
#         sortindex = data_df.isnull().sum().sort_values().index
...
#
#
# class OrderedMissingValueFiller:
#     def __init__(self, data_df, fillCols="", fill_methods=""):
#         self.original_data = data_df.copy()
#         self.missing_value_dF = data_df.copy()
#         self.fillCols = fillCols    # 缺失值填补列名称，列表类型
