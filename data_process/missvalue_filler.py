...
class ContinuousMissingValueFiller:
    ...
    def kmeans_group_fill_method(self, corrCol, fillCols=None, n_clusters=5, only_missValue=True):
        """K-means分层均值缺失值填补方法
    ...
    def sequential_kmeans_group_fill_method(self, clusterCol, fillCols=None, fill_method="ffill", n_clusters=5):
        """K-means分层均值缺失值填补方法
...
class CategoricalMissingValueFiller:
    ...
    # object类型列进行类别转化
    def change_object_cols(self, fillCols=None):
        data_df = self.missing_value_dF
        if fillCols is None:
            fillCols = self.fillCols  # 进行缺失值和异常值填补（如0值）的列索引名称，列表形式
